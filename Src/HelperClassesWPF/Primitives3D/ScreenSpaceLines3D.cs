...
namespace Game.HelperClassesWPF.Primitives3D
{
    ...
    /// </summary>
    /// 
    public class ScreenSpaceLines3D : ModelVisual3D, ISupportInitialize, IDisposable
    {
        #region Events

        //TODO: Improve this but niot updating an event list.
        public event EventHandler RebuildingGeometry;

        #endregion

        #region Declaration Section

        protected int _isInitialising;
        private Matrix3D _visualToScreen;
        private Matrix3D _screenToVisual;
        private readonly GeometryModel3D _content;
        private readonly MeshGeometry3D _mesh;		// the mesh gets placed in the content's geometry property
        ...
        #region DependencyProperty: Thickness
        ...
        {
            ((ScreenSpaceLines3D)sender).InvalidateGeometry();
        }
        ...
        #region DependencyProperty: Points
        ...
        #region DependencyProperty: IsVisible

        public static DependencyProperty IsVisibleProperty =
            DependencyProperty.Register(
                "IsVisible",
                typeof(bool),
                typeof(ScreenSpaceLines3D), new PropertyMetadata(true, OnIsVisibleChanged));
        ...
        /// <summary>
        /// This is only needed if autoUpdate is false when calling the constructor.  Call this whenever your camera moves.
        /// </summary>
        ...
        public void Render()
        {
            if (RebuildingGeometry != null)
                RebuildingGeometry(this, EventArgs.Empty);
            ...
            //_renderCount++;
            //_renderCount = _renderCount % 10000;
        ...
        internal void InvalidateGeometry()
        {
            if (_isInitialising == 0)
            {
                ...
                _visualToScreen = Math3D.ZeroMatrix;
            }
        }
        ...
        private void WireframeHelper(IList<Point3D> positions, IList<int> indices)
        {
            BeginInit();
            ...
            {
                for (int i = 2, count = positions.Count; i < count; i += 3)
                {
                    int i0 = i - 2;
                    int i1 = i - 1;
                    int i2 = i;
        ...
        public void AddPolygon(bool closeEnd, params Point3D[] points)
        {
            int i = 0;
            for (int count = points.Length - 1; i < count; i++)
            {
                Points.Add(points[i]);
        ...
        private void AddTriangle(IList<Point3D> positions, int i0, int i1, int i2)
        {
