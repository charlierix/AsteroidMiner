...
namespace Game.HelperClassesWPF
{
    public static class Math3D
    {
        ...
        public static class EvenDistribution
        {
            ...
            {
                axis = axis.ToUnit();
                Vector3D orth = Math3D.GetArbitraryOrhonganal(axis).ToUnit();
        ...
        private static class QuickHull3D
        {
            ...
            private static void ProcessTriangle_Remove(TriangleIndexedLinked triangle, SortedList<TriangleIndexedLinked, int> removedTriangles, SortedList<TriangleIndexedLinked, List<TriangleEdge>> removedRim, Vector3D farPoint)
            {
            ...
            private static void ProcessTriangle_Remove_Neighbor(TriangleIndexedLinked triangle, SortedList<TriangleIndexedLinked, int> removedTriangles, SortedList<TriangleIndexedLinked, List<TriangleEdge>> removedRim, Vector3D farPoint, TriangleIndexedLinked fromTriangle)
            {
            ...
            private static void ProcessTriangle_RemoveFromHull(List<TriangleWithPoints> hull, IList<TriangleIndexedLinked> trianglesToRemove, SortedList<TriangleIndexedLinked, List<TriangleEdge>> removedRim)
            {
            ...
            private static void ProcessTriangle_New(List<TriangleWithPoints> hull, int fartherstIndex, SortedList<TriangleIndexedLinked, List<TriangleEdge>> removedRim, Point3D[] allPoints, List<int> outsidePoints)
            {
            ...
            private static Point3D ProcessTriangle_New_InsidePoint(TriangleIndexedLinked rimTriangle, List<TriangleEdge> sharedEdges)
            {
        ...
        private static class SliceTriangles
        {
            ...
            private static ITriangle GetTangentPlane(int index, ITriangleIndexed[] triangles)
            {
                ...
                if (plane == null)
                {
                    ...
                    {
                        return new Triangle(requestPoint, triangles[0].AllPoints[otherIndices[0]], triangles[0].AllPoints[otherIndices[1]]);
                    }
                    else
                    {
                        Debug3DWindow window = new Debug3DWindow();
                        ...
                        {
                            window.AddTriangle(triangle, UtilityWPF.GetRandomColor(64, 192));
                        }
        ...
        public static Vector3D[] GetRandomVectors_Cone_EvenDist(int returnCount, Vector3D axis, double angle, double heightMin, double heightMax, double[] movableRepulseMultipliers = null, Vector3D[] existingStaticPoints = null, double[] staticRepulseMultipliers = null, double stopRadiusPercent = .004, int stopIterationCount = 1000)
        {
            return EvenDistribution.GetRandomVectors_Cone_EvenDist(returnCount, axis, angle, heightMin, heightMax, movableRepulseMultipliers, existingStaticPoints, staticRepulseMultipliers, stopRadiusPercent, stopIterationCount);
        }
        public static Vector3D[] GetRandomVectors_Cone_EvenDist(Vector3D[] movable, Vector3D axis, double angle, double heightMin, double heightMax, double[] movableRepulseMultipliers = null, Vector3D[] existingStaticPoints = null, double[] staticRepulseMultipliers = null, double stopRadiusPercent = .004, int stopIterationCount = 1000)
        {
        ...
        public static Quaternion GetRotation(Vector3D from, Vector3D to)
        {
            ...
            if (axis.IsNearZero())
            {
                ...
                else
                {
                    ...
                    axis = Math3D.GetArbitraryOrhonganal(from);
                }
            }
        ...
        public static Tetrahedron[] GetDelaunay(Point3D[] points, int? retryCountIfCorrupt = null)
        {
            return UtilityCore.RetryWrapper(points, retryCountIfCorrupt ?? 0,

                pts => DelaunayVoronoi3D.GetDelaunay(points),
        ...
        public static VoronoiResult3D GetVoronoi(Point3D[] points, bool buildFaces, int? retryCountIfCorrupt = null)
        {
            return UtilityCore.RetryWrapper(points, retryCountIfCorrupt ?? 0,

                pts => DelaunayVoronoi3D.GetVoronoi(pts, buildFaces),
        ...
        public static Point3D[] GetIntersection_Face_Sphere(Face3D face, Point3D sphereCenter, double sphereRadius)
        {
        ...
        /// </returns>
        public static Tuple<Point3D, double> GetIntersection_Plane_Sphere(ITriangle plane, Point3D sphereCenter, double sphereRadius)
        {
        ...
        */
        ...
        private static IEnumerable<Point3D> GetIntersectionFaceSphere_edge(Point3D inside, Point3D outside, TransformsToFrom2D transform2D, Point circleCenter, double circleRadius, double sphereRadius_TEMP)
        {
            ...
            //ITriangle plane = new Triangle(transform2D.Item2.Transform(new Point3D(0, 0, 0)), transform2D.Item2.Transform(new Point3D(1, 0, 0)), transform2D.Item2.Transform(new Point3D(0, 1, 0)));
    ...
    public class Face3D
    {
        ...
        public ITriangle GetPlane()
        {
            ...
            Point3D point0 = Edge3D.GetOtherPointExt(this.Edges[0], this.Edges[1]);
            Point3D point1 = this.Edges[0].GetPoint(common);
            Point3D point2 = Edge3D.GetOtherPointExt(this.Edges[1], this.Edges[0]);
