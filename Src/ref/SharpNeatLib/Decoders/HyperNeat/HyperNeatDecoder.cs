...
namespace SharpNeat.Decoders.HyperNeat
{
    ...
    public class HyperNeatDecoder : IGenomeDecoder<NeatGenome,IBlackBox>
    {
        ...
        /// <summary>
        /// This is a copy of Decode, but returns a network definition that can be handed to the genome viewer
        /// </summary>
        public Tuple<IBlackBox, INetworkDefinition> Decode2(NeatGenome genome)
        {
            // Decode the CPPN.
            IBlackBox cppn = _decodeCppnMethod(genome);
            ...
            // Create a network from the substrate network definition, and return it.
            return Tuple.Create(
                _createSubstrateNetworkMethod(substrateNetworkDef),
                substrateNetworkDef);
        }
