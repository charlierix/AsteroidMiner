...
namespace SharpNeat.EvolutionAlgorithms
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        /// <summary>
        /// This is used to hold off initialization until the first time step is called
        /// </summary>
        ...
        public NeatEvolutionAlgorithm()
            : this(new NeatEvolutionAlgorithmParameters(), new KMeansClusteringStrategy<TGenome>(new ManhattanDistanceMetric()), new NullComplexityRegulationStrategy())
        {
        }
        ...
        {
            //NOTE: Can't do this now, it must be done during the first time step is called.  Initialize is called from the main thread, not the worker thread
            //
            // I think this was done so the properties are usable right away, no need to wait for the timer
            return;
        ...
        protected override void PerformOneGeneration()
        {
            if (_isFirst)
            {
                ...
                // This section used to be called from the initialize method, but that was called from the main thread, so moved it here
            ...
            SpecieStats[] specieStatsArr = CalcSpecieStats(out int offspringCount);
