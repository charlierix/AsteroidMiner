...
namespace Game.Newt.AsteroidMiner2
{
    ...
    public static class MutateUtility
    {
        ...
        public static ShipDNA Mutate(ShipDNA dna, ShipMutateArgs args)
        {
            ...
            {
                MutateNeural(retVal, args.NeuronChanges);
            }
        ...
        {
            return value + Math3D.GetRandomVectorSphericalShell(value.Length * factor);
        }
        public static Vector3D Mutate_RandPercent(Vector3D value, double factor)
        {
            return value + Math3D.GetRandomVectorSpherical(value.Length * factor);
        }
        public static Vector3D Mutate_RandPercent(out double remainder, Vector3D value, double factor)
        {
            double length = value.Length;

            Vector3D offset = Math3D.GetRandomVectorSpherical(length * factor);
        ...
        public static Quaternion Mutate_FixedPercent(Quaternion value, double factor)
        {
            if (StaticRandom.Next(2) == 0)
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), factor * 360d));
            }
            else
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), -factor * 360d));
        ...
        {
            return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), Math3D.GetNearZeroValue(factor * 360d)));
        }
        public static Quaternion Mutate_RandPercent(out double remainder, Quaternion value, double factor)
        {
            ...
            if (StaticRandom.Next(2) == 0)
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), randFactor * 360d));
            }
            else
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), -randFactor * 360d));
        ...
        public static Quaternion Mutate_FixedDistance(Quaternion value, double factor)
        {
            if (StaticRandom.Next(2) == 0)
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), factor));
            }
            else
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), -factor));
        ...
        {
            return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), Math3D.GetNearZeroValue(factor)));
        }
        public static Quaternion Mutate_RandDistance(out double remainder, Quaternion value, double factor)
        {
            ...
            if (StaticRandom.Next(2) == 0)
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), randFactor));
            }
            else
            {
                return value.RotateBy(new Quaternion(Math3D.GetRandomVectorSphericalShell(1d), -randFactor));
        ...
        private static void MutateNeural(ShipDNA retVal, NeuronMutateArgs args)
        {
            // Get all the parts that are dna containers
            PartNeuralDNA[] parts = retVal.PartsByLayer.SelectMany(o => o.Value).Where(o => o is PartNeuralDNA).Select(o => (PartNeuralDNA)o).ToArray();
            ...
            {
                PropsByPercent props = new PropsByPercent(parts.SelectMany(o => UtilityHelper.Iterate(o.InternalLinks, o.ExternalLinks)));		// internal and external links are complex types, so make the list iterate over each of them to get at the primitive props inside
    ...
    public class PropsByPercent : IEnumerable<PropsByPercent.PropWrapper>
    {
        ...
        private static ClassTracker[] ParseList(object[] list, FilterArgs filter)
        {
            ...
            List<Tuple<int, PercentTracker[], Type>> counts = new List<Tuple<int, PercentTracker[], Type>>();
