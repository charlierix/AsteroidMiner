...
namespace Game.Newt.AsteroidMiner2
{
    ...
    /// property needs to be set directly by the sensor, SetValue is ignored
    /// </summary>
    public class Neuron_SensorPosition : INeuron
    {
        public Neuron_SensorPosition(Point3D position, bool isPositiveOnly)
        {
            _position = position;
    ...
    public class NeuralBucket
    {
        ...
        public readonly long Token = TokenGenerator.Instance.NextToken();
    ...
    public class NeuralPool
    {
        ...
        private class TaskWrapper : IDisposable
        {
            ...
            public readonly List<NeuralBucket> Adds = new List<NeuralBucket>();
            public readonly List<NeuralBucket> Removes = new List<NeuralBucket>();
            ...
            private static void Run(TaskWrapper parent, CancellationToken cancel)
            {
                ...
                try
                {
                    ...
                    while (!cancel.IsCancellationRequested)
                    {
                        ...
                        lock (parent.Lock)
                        {
                            if (parent.Removes.Count > 0 || parent.Adds.Count > 0)
                            {
                                // Remove
                                foreach (NeuralBucket bucket in parent.Removes)
                                {
                                    buckets.Remove(bucket);
                                }
                                parent.Removes.Clear();

                                // Add
                                buckets.AddRange(parent.Adds);
                                parent.Adds.Clear();
                        ...
                        foreach (NeuralBucket bucket in buckets)
                        {
                            bucket.Tick();
                        }
    ...
    public static class NeuralUtility
    {
        ...
        public static void PopulateDNALinks(PartNeuralDNA dna, INeuronContainer dnaSource, IEnumerable<ContainerOutput> outputs)
        {
