...
{
	//TODO: There will be a camera eye, and rangefinder eye
	...
	public class EyeToolItem : PartToolItemBase
	{
		...
		{
			return new EyeDesign(this.Options);
		}
	...
	public class EyeDesign : PartDesignBase
	{
		...
		public EyeDesign(EditorOptions options)
			: base(options) { }
		...
		private Model3DGroup CreateGeometry(bool isFinal)
		{
			...
			if (!isFinal)
			{
				...
				this.MaterialBrushes.Add(new MaterialColorProps(diffuse, diffuse.Brush, this.Options.WorldColors.EyeLens));		//	using the final's lens color, because it's a solid color
			...
			if (isFinal)
			{
				material.Children.Add(new DiffuseMaterial(new SolidColorBrush(this.Options.WorldColors.EyeLens)));		//	no need to add these to this.MaterialBrushes (those are only for editing)
				material.Children.Add(this.Options.WorldColors.EyeLensSpecular);
			}
			else
			{
			...
			geometry = new GeometryModel3D();
			material = new MaterialGroup();
			diffuse = new DiffuseMaterial(new SolidColorBrush(this.Options.WorldColors.EyeBase));
			this.MaterialBrushes.Add(new MaterialColorProps(diffuse, diffuse.Brush, this.Options.WorldColors.EyeBase));
			material.Children.Add(diffuse);
			specular = this.Options.WorldColors.EyeBaseSpecular;
			this.MaterialBrushes.Add(new MaterialColorProps(specular));
			material.Children.Add(specular);
