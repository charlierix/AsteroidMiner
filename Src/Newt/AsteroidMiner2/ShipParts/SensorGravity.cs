...
namespace Game.Newt.AsteroidMiner2.ShipParts
{
    ...
    public class SensorGravity : PartBase, INeuronContainer, IPartUpdatable
    {
        ...
        private IGravityField _field = null;
        ...
        private MagnitudeHistory _magnitudeHistory = new MagnitudeHistory();

        #endregion

        #region Constructor

        public SensorGravity(EditorOptions options, ItemOptions itemOptions, PartNeuralDNA dna, IContainer energyTanks, IGravityField field)
            : base(options, dna)
        {
        ...
        internal static void UpdateNeurons(Neuron_SensorPosition[] neurons, double neuronMaxRadius, Vector3D vector, double magnitude)
        {
            ...
            for (int cntr = 0; cntr < neurons.Length; cntr++)
            {
                ...
                else
                {
                    ...
                    double revisedMinDot = UtilityHelper.GetScaledValue_Capped(0d, MINDOT, 0d, 1d, radiusPercent);
                    ...
                    {
                        percent = UtilityHelper.GetScaledValue_Capped(0d, minReturn, 0d, revisedMinDot, dot);
                    }
                    else
                    {
                        percent = UtilityHelper.GetScaledValue_Capped(minReturn, 1d, revisedMinDot, 2d, dot);
                    }
        ...
        private void UpdateNeurons_progressiveRadius(Vector3D gravity, double magnitude)
        {
            ...
            for (int cntr = 0; cntr < _neurons.Length; cntr++)
            {
                ...
                else
                {
                    ...
                    _neurons[cntr].Value = UtilityHelper.GetScaledValue_Capped(0d, 1d, 0d, 1d, 1d - radiusPercent);
                }
            }
        }
        private void UpdateNeurons_fixed(Vector3D gravity, double magnitude)
        {
            ...
            for (int cntr = 0; cntr < _neurons.Length; cntr++)
            {
                ...
                else
                {
                    ...
                    {
                        _neurons[cntr].Value = UtilityHelper.GetScaledValue_Capped(0d, 1d, MINDOT, 2d, dot);
                    }
                }
            }
        }
