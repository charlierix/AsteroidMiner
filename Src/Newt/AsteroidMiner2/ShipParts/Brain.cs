...
namespace Game.Newt.AsteroidMiner2.ShipParts
{
    ...
    public class BrainDesign : PartDesignBase
    {
        ...
        public override PartDNA GetDNA()
        {
            ...
            base.FillDNA(retVal);
            //retVal.Neurons = this.NeuronLocations;
        ...
        public override void SetDNA(PartDNA dna)
        {
            ...
            //PartNeuralDNA dnaCast = (PartNeuralDNA)dna;
            //this.NeuronLocations = PartNeuralDNA.Neurons;
        }
    ...
    public class Brain : PartBase, INeuronContainer, IPartUpdatable
    {
        ...
        /// </summary>
        private Neuron_NegPos[] _neurons = null;

        /// <summary>
        /// These represent brain chemicals, and are implemented as writeonly neurons
        /// </summary>
        private Neuron_Fade[] _brainChemicals = null;
        ...
        public Brain(EditorOptions options, ItemOptions itemOptions, PartNeuralDNA dna, IContainer energyTanks)
            : base(options, dna)
        {
        ...
        public IEnumerable<INeuron> Neruons_All
        {
            get
            {
                return UtilityHelper.Iterate<INeuron>(_neurons, _brainChemicals);
            }
        }
        ...
        {
            PartNeuralDNA retVal = this.Design.GetDNA() as PartNeuralDNA;
            if (retVal == null)
            {
                throw new ApplicationException("Couldn't cast dna as PartNeuralDNA");
            }
        ...
        public static Vector3D[] GetNeuronPositions_Cluster(Point3D[] dnaPositions, Point3D[] existingStaticPositions, double staticMultValue, int count, double radius, double minClusterDistPercent)
        {
            ...
            Vector3D[] retVal = GetNeuronPositionsInitial(out staticPoints, out staticRepulseMult, dnaPositions, existingStaticPositions, staticMultValue, count, radius, o => Math3D.GetRandomVectorSpherical(o));

            // Don't let them get too close to each other
            retVal = Math3D.GetRandomVectorsSphericalClusteredMinDist(retVal, staticPoints, radius, radius * 2d * minClusterDistPercent, .03d, 1000, null, staticRepulseMult);		//clustdist% is of diameter, not radius
        ...
        public static Vector3D[] GetNeuronPositions_Even(Point3D[] dnaPositions, Point3D[] existingStaticPositions, double staticMultValue, int count, double radius)
        {
            ...
            retVal = Math3D.GetRandomVectorsSphericalEvenDist(retVal, staticPoints, radius, .03d, 1000, null, staticRepulseMult);
        ...
        public static Vector3D[] GetNeuronPositions_Even2D(Point3D[] dnaPositions, Point3D[] existingStaticPositions, double staticMultValue, int count, double radius, double z = 0)
        {
            ...
            Vector3D[] staticPoints;
            double[] staticRepulseMult;
            Vector3D[] retVal = GetNeuronPositionsInitial(out staticPoints, out staticRepulseMult, dnaPositions, existingStaticPositions, staticMultValue, count, radius, o => Math3D.GetRandomVectorSpherical2D(o));
            ...
            // Space them out evenly
            retVal2D = Math3D.GetRandomVectorsCircularEvenDist(retVal2D, staticPoints2D, radius, .03d, 1000, null, staticRepulseMult);
        ...
        private static Neuron_Fade[] CreateBrainChemicals(PartNeuralDNA dna, ItemOptions itemOptions)
        {
        ...
        private static Neuron_NegPos[] CreateNeurons(PartNeuralDNA dna, ItemOptions itemOptions, Point3D[] brainChemicalPositions)
        {
            // Figure out how many to make
            double radius, volume;
            GetNeuronVolume(out radius, out volume, dna, itemOptions);
        ...
        private static void GetNeuronVolume(out double radius, out double volume, PartNeuralDNA dna, ItemOptions itemOptions)
        {
