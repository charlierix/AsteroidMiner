...
namespace Game.Newt.AsteroidMiner2.ShipParts
{
    ...
    public class CameraColorRGB : PartBase, INeuronContainer, IPartUpdatable, ICameraPoolCamera
    {
        ...
        private Neuron_SensorPosition[] _neuronsR = null;
        private Neuron_SensorPosition[] _neuronsG = null;
        private Neuron_SensorPosition[] _neuronsB = null;

        private OverlayResult[][] _overlayR = null;
        private OverlayResult[][] _overlayG = null;
        private OverlayResult[][] _overlayB = null;
        ...
        public CameraColorRGB(EditorOptions options, ItemOptions itemOptions, PartNeuralDNA dna, IContainer energyTanks, CameraPool cameraPool)
            : base(options, dna)
        {
        ...
        public IEnumerable<INeuron> Neruons_Readonly
        {
            get
            {
                return UtilityHelper.Iterate(_neuronsR, _neuronsG, _neuronsB);
            }
        }
        ...
        private int _pixelWidthHeight = 1;
        ...
        internal static void CreateNeurons(out Neuron_SensorPosition[] neuronsR, out Neuron_SensorPosition[] neuronsG, out Neuron_SensorPosition[] neuronsB, out OverlayResult[][] overlayR, out OverlayResult[][] overlayG, out OverlayResult[][] overlayB, out int pixelWidthHeight, PartNeuralDNA dna, ItemOptions itemOptions, double neuronDensity)
        {
        ...
        internal static Vector3D[][] GetNeuronPositions(Point3D[] dnaPositions, int count, int numPlates, double radius)
        {
            ...
            for (int cntr = 0; cntr < retVal.Length; cntr++)
            {
                ...
                evenPoints = Math3D.GetRandomVectorsCircularCenterPacked(evenPoints, null, radius, .03d, 1000, null, null);
        ...
        private static Vector3D[][] GetNeuronPositionsInitial(Point3D[] dnaPositions, int count, int numPlates, double radius)
        {
            ...
            if (dnaPositions == null)
            {
                ...
                for (int plateCntr = 0; plateCntr < numPlates; plateCntr++)
                {
                    ...
                    {
                        plate[cntr] = Math3D.GetRandomVectorSpherical2D(radius);
            ...
            else
            {
                ...
                else if (dnaPositions.Length < count)
                {
                    ...
                    for (int cntr = 0; cntr < increaseCount; cntr++)
                    {
                        ...
                        Vector3D newNeuron = Math3D.GetRandomVectorSpherical2D(radius);
        ...
        private static int[] GetNeuronPositionsInitialSprtInitialPlateBreakdown(int count, int numPlates)
        {
            ...
            retVal = UtilityHelper.RandomRange(0, numPlates).Select(o => retVal[o]).ToArray();
