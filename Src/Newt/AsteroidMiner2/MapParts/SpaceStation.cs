...
namespace Game.Newt.AsteroidMiner2.MapParts
{
	public class SpaceStation : IMapObject
	{
		...
		private WorldColors _colors = null;
		...
		public SpaceStation(Point3D position, World world, int materialID, WorldColors colors)
		{
			//TODO:  Windows, lights
			...
			Color hullColor = colors.SpaceStationHull;
			SpecularMaterial hullSpecular = colors.SpaceStationHullSpecular;
			...
			Brush skinBrush = new SolidColorBrush(colors.SpaceStationGlass);  // the skin is semitransparent, so you can see the components inside

			material = new MaterialGroup();
			material.Children.Add(new DiffuseMaterial(skinBrush));
			material.Children.Add(colors.SpaceStationGlassSpecular_Front);     // more reflective (and white light)

			MaterialGroup backMaterial = new MaterialGroup();
			backMaterial.Children.Add(new DiffuseMaterial(skinBrush));
			backMaterial.Children.Add(colors.SpaceStationGlassSpecular_Back);       // dark light, and not very reflective

			//	Geometry Model
			geometry = new GeometryModel3D();
			geometry.Material = material;
			geometry.BackMaterial = backMaterial;
			geometry.Geometry = UtilityWPF.GetSphere(6, radius, radius, radius * .25);
			...
			geometry.Material = material;
			geometry.BackMaterial = backMaterial;
			geometry.Geometry = UtilityWPF.GetSphere(6, forceFieldSize.X, forceFieldSize.Y, forceFieldSize.Z);
			...
			CollisionHull hull = CollisionHull.CreateSphere(world, 0, forceFieldSize, null);
		...
		private void Timer_Tick(object sender, EventArgs e)
		{
			...
			if (_forceFieldOpacity > 0d)
			{
				...
				//	Set the colors
				_forceField_DiffuseMaterial.Brush = new SolidColorBrush(UtilityWPF.AlphaBlend(_colors.SpaceStationForceField, Colors.Transparent, _forceFieldOpacity));
				_forceField_EmissiveFront.Brush = new SolidColorBrush(UtilityWPF.AlphaBlend(_colors.SpaceStationForceFieldEmissive_Front, Colors.Transparent, _forceFieldOpacity));
				_forceField_EmissiveRear.Brush = new SolidColorBrush(UtilityWPF.AlphaBlend(_colors.SpaceStationForceFieldEmissive_Back, Colors.Transparent, _forceFieldOpacity));
