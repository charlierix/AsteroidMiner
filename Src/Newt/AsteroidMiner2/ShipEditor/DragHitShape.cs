...
namespace Game.Newt.AsteroidMiner2.ShipEditor
{
	...
	public class DragHitShape
	{
		...
		public Point3D? CastRay(RayHitTestParameters ray)
		{
			...
			switch (_shape)
			{
				case ShapeType.Line:
					...
					if (Math3D.GetClosestPointsBetweenLines(out point1, out point2, _point, _direction, ray.Origin, ray.Direction))
					{
						retVal = point1.Value;
					}
				...
				case ShapeType.Plane:
					...
					retVal = Math3D.GetIntersectionOfPlaneAndLine(_plane, ray.Origin, ray.Direction);
				...
				case ShapeType.Circle:
					...
					if (Math3D.GetClosestPointsBetweenLineCircle(out nearestCirclePoints, out nearestLinePoints, _plane, _point, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
					{
				...
				case ShapeType.Cylinder:
					...
					if (Math3D.GetClosestPointsBetweenLineCylinder(out nearestCylinderPoints, out nearestLinePoints2, _point, _direction, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
					{
				...
				case ShapeType.Sphere:
					...
					Math3D.GetClosestPointsBetweenLineSphere(out nearestSpherePoints, out nearestLinePoints3, _point, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay);
					{
		...
		/// </remarks>
		internal Vector3D? GetNormal(Point3D point)
		{
			throw new ApplicationException("finish this");
		}
		...
		private static Point3D? CastRay_Line(out double clickDistanceSquared, Point3D point, Vector3D direction, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
		{
			...
			if (Math3D.GetClosestPointsBetweenLines(out point1, out point2, point, direction, mouseDownClickRay.Origin, mouseDownClickRay.Direction) &&
				Math3D.GetClosestPointsBetweenLines(out point3, out point4, point, direction, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction))
			{
			...
			if (Math3D.GetClosestPointsBetweenLines(out point1, out point2, point, direction, currentClickRay.Origin, currentClickRay.Direction))
			{
		...
		private static Point3D? CastRay_Plane(ITriangle plane, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
		{
			...
			Point3D? mouseDownClickPoint = Math3D.GetIntersectionOfPlaneAndLine(plane, mouseDownClickRay.Origin, mouseDownClickRay.Direction);
			Point3D? mouseDownCenterPoint = Math3D.GetIntersectionOfPlaneAndLine(plane, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction);
			...
			Point3D? retVal = Math3D.GetIntersectionOfPlaneAndLine(plane, currentClickRay.Origin, currentClickRay.Direction);
		...
		private static Point3D? CastRay_Circle(out double clickDistanceSquared, ITriangle plane, Point3D center, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
		{
			...
			if (Math3D.GetClosestPointsBetweenLineCircle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
			{
				clickDistanceSquared = (nearestLinePoints[0] - nearestCirclePoints[0]).LengthSquared;
				currentClickPoint = nearestCirclePoints[0];

				if (Math3D.GetClosestPointsBetweenLineCircle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, mouseDownClickRay.Origin, mouseDownClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
				{
					mouseDownClickPoint = nearestCirclePoints[0];

					if (Math3D.GetClosestPointsBetweenLineCircle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction, Math3D.RayCastReturn.ClosestToRay))
					{
		...
		private static Point3D? CastRay_Cylinder(Point3D point, Vector3D direction, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
		{
			...
			if (Math3D.GetClosestPointsBetweenLineCylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
			{
				currentClickPoint = nearestCylinderPoints[0];

				if (Math3D.GetClosestPointsBetweenLineCylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, mouseDownClickRay.Origin, mouseDownClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
				{
					mouseDownClickPoint = nearestCylinderPoints[0];

					if (Math3D.GetClosestPointsBetweenLineCylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction, Math3D.RayCastReturn.ClosestToRay))
					{
			...
			Point3D modelCenter = Math3D.GetNearestPointAlongLine(point, direction, currentClickPoint.Value);
		...
		private static Point3D? CastRay_Sphere(Point3D center, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
		{
			...
			Math3D.GetClosestPointsBetweenLineSphere(out nearestSpherePoints, out nearestLinePoints, center, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay);
		...
		private static Point3D? CastRay_LinesCircles(IEnumerable<RayHitTestParameters> lines, IEnumerable<CircleDefinition> circles, RayHitTestParameters ray)
		{
			...
			if (lines != null)
			{
				...
				{
					Point3D? point1, point2;
					if (Math3D.GetClosestPointsBetweenLines(out point1, out point2, line.Origin, line.Direction, ray.Origin, ray.Direction))
					{
						double localDistance = (point2.Value - point1.Value).Length;
			...
			if (circles != null)
			{
				...
				foreach (CircleDefinition circle in circles)
				{
					...
					if (Math3D.GetClosestPointsBetweenLineCircle(out points1, out points2, circle.Plane, circle.Center, circle.Radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
					{
						double localDistance = (points2[0] - points1[0]).Length;
						if (retVal == null || localDistance < distance)
						{
							retVal = points1[0];
							distance = localDistance;
		...
		private static Quaternion GetRotation_Circle(Point3D center, Vector3D normal, Point3D from, Point3D to)
		{
			...
			Vector3D fromLine = from - Math3D.GetNearestPointAlongLine(center, normal, from);
			Vector3D toLine = to - Math3D.GetNearestPointAlongLine(center, normal, to);
