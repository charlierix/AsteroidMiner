...
namespace Game.Newt.AsteroidMiner2.ShipEditor
{
	...
	public class DraggableModifierSphere : DraggableModifierBase
	{
		...
		private Model3D GetBall()
		{
			...
			retVal.Geometry = UtilityWPF.GetSphere(20, .1);
	...
	public class DraggableModifierRing : DraggableModifierBase
	{
		...
		public override void StartDrag(RayHitTestParameters ray)
		{
			...
			if (Math3D.GetClosestPointsBetweenLineCircle(out nearestCirclePoints, out nearestLinePoints, _circlePlane, _circleCenter.Value, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
			{
		...
		{
			double size = UtilityHelper.GetScaledValue_Capped(.025d, .15d, .1d, 10d, _radius);
