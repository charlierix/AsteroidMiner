...
{
    //TODO: This uses events, which seems to only work for the UI thread.  I can't figure out how to make a background thread that stays alive and
    //can listen to events.  I think a background thread that own its own world would need to run in a loop, and tell World when to run (then optionally
    //could thread.sleep in between?)
    //
    //If that is the case, make a constructor overload that won't use the timer
    ...
    public class World : WorldBase
    {
        ...
        /// <param name="useTimer">
        /// True=World creates its own timer (probably only works in UI thread)
        /// False=World doesn't create a timer.  Instead, this.Update must be called on a regular basis (use this option when creating a world from a long running background thread)
        /// </param>
        public World(bool useTimer = true)
            : base()
        {
    ...
    public abstract class WorldBase : IDisposable
    {
        ...
        protected WorldBase()
        {
            ...
            ObjectStorage.Instance.AddWorld(this);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                ObjectStorage.Instance.RemoveWorld(this);
    ...
    {
        public BodyDestroyedArgs(Body body)
        {
            this.Body = body;
        }
