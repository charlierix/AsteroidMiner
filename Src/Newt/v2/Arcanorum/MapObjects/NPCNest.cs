...
namespace Game.Newt.v2.Arcanorum.MapObjects
{
    ...
    /// </remarks>
    public class NPCNest : IDisposable, IMapObject, ISensorVisionPoints, IPartUpdatable
    {
        ...
        private readonly List<(GeometryModel3D model, Point3D position, double createTime)> _eggs = new List<(GeometryModel3D, Point3D, double)>();
        ...
        public NPCNest(NPCNestDNA dna, double radius, World world, Map map, KeepItems2D keepItems2D, MaterialIDs materialIDs, Viewport3D viewport, EditorOptions editorOptions, ItemOptionsArco itemOptions, IGravityField gravity, DragHitShape dragPlane)
        {
            ...
            _dreamer.WeaponDNA = EvolutionDreamer.GetRandomDNA(_itemOptions).Item2;

            #region WPF Model

            var models = GetModel(_shellColors, radius);
            this.Model = models.nest;
            _eggModels = models.eggs;
            ...
            _sensorVisionPoints = GetSensorVisionPoints_Model(radius);
        ...
        public bool IsDisposed => _isDisposed;// || this.PhysicsBody.IsDisposed;       // the nest doesn't have a body

        public Body PhysicsBody => null;

        public Visual3D[] Visuals3D { get; private set; }
        ...
        public Vector3D VelocityWorld => new Vector3D(0, 0, 0);
        public Matrix3D OffsetMatrix => throw new InvalidOperationException("This class doesn't have a physics body");
        ...
        {
            Point3D position = _dragPlane.CastRay(PositionWorld) ?? PositionWorld;      // cast ray potentially returns null, but that should never happen

            TranslateTransform3D transform = new TranslateTransform3D(position.ToVector());

            Point3D[] points = _sensorVisionPoints.ToArray();       // make a copy
        ...
        private void Update_AddBot(ArcBotNPC[] currentBots)
        {
            ...
            else if (_eggs[0].createTime > _time - 4)        // item 0 will always be the oldest egg.  Time is in seconds.  TODO: Hatch time should come from nest DNA, or calculated based on bot complexity (some nests may build lots of simple bots)
            {
            ...
            // Remove the egg
            _eggModels.Children.Remove(_eggs[0].model);
            _eggs.RemoveAt(0);
            ...
            ArcBotNPC bot = new ArcBotNPC(dna.bot, level, position.Value, _world, _map, _keepItems2D, _materialIDs, _viewport, _editorOptions, _itemOptions, _gravity, _dragPlane, this.PositionWorld, HOMINGRADIUS, true, true)
            {
                NestToken = Token,
            };
            ...
            if (dna.Item2 != null)
            {
                Weapon weapon = new Weapon(dna.weapon, new Point3D(0, 0, 0), _world, _materialIDs.Weapon);
        ...
        private void Update_AddEgg(double elapsedTime)
        {
            ...
            Point3D position = GetNewEggPosition(_eggs.Select(o => o.position), eggRadius, this.Radius);

            GeometryModel3D geometry = GetEggModel(position, eggRadius, _shellColors);
            _eggModels.Children.Add(geometry);

            _eggs.Add((geometry, position, _time));
        ...
        private static (Model3DGroup nest, Model3DGroup eggs) GetModel(BotShellColorsDNA colors, double radius)
        {
        ...
        private Point3D[] GetSensorVisionPoints_Model(double radius)
        {
            const double REDUCE = .85;

            return Math2D.GetCircle_Cached(4).
                Select(o => (o.ToVector3D() * (radius * REDUCE)).ToPoint()).
                //Concat(new[] { new Point3D(0, 0, 0) }).       // this is unnecessary
                ToArray();
        }
