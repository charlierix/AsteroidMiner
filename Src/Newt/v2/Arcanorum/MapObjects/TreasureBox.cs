...
namespace Game.Newt.v2.Arcanorum.MapObjects
{
    ...
    //  TreasureBox
    //  Safe
    public class TreasureBox : IMapObject, ITakesDamage, ISensorVisionPoints, IDisposable
    {
        ...
        {
            return Weapon.GetSensorVisionPoints(_sensorVisionPoints, PhysicsBody);
        }
        ...
        private Point3D[] GetSensorVisionPoints_Model(double radius, double height)
        {
            const double REDUCE = .85;

            double heightReduced = (height / 2) * REDUCE;
            double radiusReduced = radius * REDUCE;

            return new[]
            {
                new Point3D(-heightReduced, -radiusReduced, 0),
                new Point3D(-heightReduced, radiusReduced, 0),
