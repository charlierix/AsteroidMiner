...
namespace Game.Newt.v2.Arcanorum.MapObjects
{
    ...
    //TODO: Add a MapObject that is food.  It would be stored in a cargobox and replenish the energy tank.  Maybe trade it in for player upgrades
    ...
    public class ArcBot : IMapObject, IPartUpdatable, IGivesDamage, ITakesDamage, IDisposable
    {
        ...
        #endregion
        #region class: ConstructResult

        private class ConstructResult
        {
            public PartContainer Container { get; set; }
            public ShipPartDNA[] DNA { get; set; }
            public CollisionHull[] Hulls { get; set; }
            public AIMousePlate MousePlate { get; set; }
        }
        ...
        private readonly Task<ConstructResult> _partsTask;
        ...
        public ArcBot(BotDNA dna, int level, Point3D position, World world, Map map, KeepItems2D keepItems2D, MaterialIDs materialIDs, Viewport3D viewport, EditorOptions editorOptions, ItemOptionsArco itemOptions, IGravityField gravity, DragHitShape dragPlane, Point3D homingPoint, double homingRadius, bool runNeural, bool repairPartPositions)
        {
            ...
            if (dna != null && dna.Parts != null)
            {
                ...
                {
                    ConstructResult result = t.Result;

                    if (result != null)
                    {
                        _parts = result.Container;
                        DNAPartial.Parts = result.DNA;        //NOTE: Parts were already cloned from the dna passed into the constructor.  This replaces with the final (throws out parts that are too small, fills in links, makes sure parts aren't intersecting)
                        DNAFinal = DNAPartial;
                        _aiMousePlate = result.MousePlate;
                        if (_aiMousePlate != null)
                        {
                            _aiMousePlate.MaxXY = Radius * 20;
                        }
                        ...
                        {
                            foreach (CollisionHull partHull in result.Hulls)
                            {
                                partHull.Dispose();
                            }
                        }
            ...
            ReceiveDamageMultipliers = new DamageProps();
            ...
            if (disposing)
            {
                ...
                if (_partsTask != null)
                {
                    ...
                    _partsTask.ContinueWith(t =>
                    {
                        ...
                        if (result != null)
                        {
                            NeuralPool.Instance.Remove(result.Container.LinkBucket);
                        }
                    });
                }
        ...
        public (bool isDead, DamageProps actualDamage)? Damage(DamageProps damage, Weapon weapon = null)
        {
            ...
            return DamageProps.DoDamage(this, damage);
        }

        public DamageProps ReceiveDamageMultipliers
        {
            get;
            private set;
        }
        ...
        public RamWeapon Ram { get; private set; }
        ...
        /// the chase point
        /// </summary>
        public MapObject_ChasePoint_Velocity DraggingBot { get; private set; }
        ...
        protected virtual void OnConstructFinished()
        {
            ...
            foreach (ShipPartDNA dna in parts)
            {
                switch (dna.PartType)
                {
                    ...
                    case SensorVision.PARTTYPE:
                        ...
                        {
                            SensorVision sensorVision = new SensorVision(editorOptions, itemOptions, sensorVisionDNA, map);
        ...
        private void LevelChanged()
        {
            ...
            if (weapon != null)
            {
                ...
                weapon = new Weapon(weapon.DNA, new Point3D(0, 0, 0), _world, _materialIDs.Weapon);
                AttachWeapon(weapon);
            }
        }
