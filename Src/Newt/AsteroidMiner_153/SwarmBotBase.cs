...
namespace Game.Newt.AsteroidMiner_153
{
	public class SwarmBotBase : IMapObject
	{
		...
		protected void CreateBot(Viewport3D viewport, SharedVisuals sharedVisuals, World world, Point3D worldPosition)
		{
			...
			_coreGeometry.Material = _coreMaterialNeutral;
			_coreGeometry.BackMaterial = _coreMaterialNeutral;
			_coreGeometry.Geometry = UtilityWPF.GetSphere(5, _radius * .4, _radius * .4, _radius * .4);
			...
			geometry.Geometry = UtilityWPF.GetSphere(5, _radius, _radius, _radius);
		...
		/// <summary>
		/// This will for the target point, but tries to get my velocity going straight to the target (tries to counter any velocity
