...
namespace Game.Newt.AsteroidMiner_153
{
	public class Mineral : IMapObject
	{
		...
		public void CreateMineral(MaterialManager materialManager, Map map, SharedVisuals sharedVisuals, Vector3D position, bool randomOrientation, double volumeInCubicMeters)
		{
			...
			{
				randomRotation = new RotateTransform3D(new AxisAngleRotation3D(Math3D.GetRandomVectorSpherical(1d), Math3D.GetNearZeroValue(360d)));
			}
		...
		private void Body_ApplyForce(Body sender, BodyForceEventArgs e)
		{
			...
			{
				e.AddImpulse(_initialVelocity, Math3D.GetRandomVectorSpherical(this.Radius * .5d));     // whacking it at an offset, so it has some rotation as well
