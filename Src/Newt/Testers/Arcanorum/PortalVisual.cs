...
namespace Game.Newt.Testers.Arcanorum
{
    public class PortalVisual : IMapObject
    {
        ...
        private Model3D GetModel(out BotShellColorsDNA backdropColors, PortalVisualType portalType, double radius)
        {
            switch (portalType)
            {
        ...
        private Model3D GetModel_Shop(out BotShellColorsDNA backdropColors, double radius)
        {
            ...
            geometry.Geometry = UtilityWPF.GetSphere(6, radius, radius, radius * .1);
            ...
            // Get gold's color
            Color mineralDiffuse, dummy1, dummy3;
            double dummy2, dummy4;
            Mineral.GetSettingsForMineralType(out mineralDiffuse, out dummy1, out dummy2, out dummy3, out dummy4, MineralType.Gold);
            backdropColors.Light = mineralDiffuse.ToHex();

            // Get the model of a gold mineral
            Model3D model = Mineral.GetNewVisual(out dummy2, MineralType.Gold);
