...
namespace Game.Newt.HelperClasses
{
    ...
    public class DragHitShape
    {
        ...
        public Point3D? CastRay(Point3D point)
        {
            ...
            switch (_shape)
            {
                case ShapeType.Line:
                    ...
                    retVal = Math3D.GetClosestPoint_Point_Line(_point, _direction, point);
                ...
                case ShapeType.Plane:
                    ...
                    retVal = Math3D.GetClosestPoint_Point_Plane(_plane, point);
                ...
                case ShapeType.Circle:
                    ...
                    retVal = Math3D.GetClosestPoint_Point_Circle(_plane, _point, _radius, point);
                ...
                case ShapeType.Cylinder:
                    ...
                    retVal = Math3D.GetClosestPoint_Point_Cylinder(_point, _direction, _radius, point);
                ...
                case ShapeType.Sphere:
                    ...
                    retVal = Math3D.GetClosestPoint_Point_Sphere(_point, _radius, point);
        ...
        public Point3D? CastRay(RayHitTestParameters ray)
        {
            ...
            switch (_shape)
            {
                ...
                case ShapeType.Circle:
                    ...
                    if (Math3D.GetClosestPoints_Line_Circle(out nearestCirclePoints, out nearestLinePoints, _plane, _point, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
                    {
                ...
                case ShapeType.Cylinder:
                    ...
                    if (Math3D.GetClosestPoints_Line_Cylinder(out nearestCylinderPoints, out nearestLinePoints2, _point, _direction, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
                    {
                ...
                case ShapeType.Sphere:
                    ...
                    Math3D.GetClosestPoints_Line_Sphere(out nearestSpherePoints, out nearestLinePoints3, _point, _radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay);
                    {
        ...
        public Vector3D? GetNormal(Point3D point)
        {
            ...
            switch (_shape)
            {
                ...
                case ShapeType.Cylinder:
                    ...
                    Point3D nearestAxisPoint = Math3D.GetClosestPoint_Point_Line(_point, _direction, point);
        ...
        private static Point3D? CastRay_Circle(out double clickDistanceSquared, ITriangle plane, Point3D center, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
        {
            ...
            if (Math3D.GetClosestPoints_Line_Circle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
            {
                clickDistanceSquared = (nearestLinePoints[0] - nearestCirclePoints[0]).LengthSquared;
                currentClickPoint = nearestCirclePoints[0];

                if (Math3D.GetClosestPoints_Line_Circle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, mouseDownClickRay.Origin, mouseDownClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
                {
                    mouseDownClickPoint = nearestCirclePoints[0];

                    if (Math3D.GetClosestPoints_Line_Circle(out nearestCirclePoints, out nearestLinePoints, plane, center, radius, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction, Math3D.RayCastReturn.ClosestToRay))
                    {
        ...
        private static Point3D? CastRay_Cylinder(Point3D point, Vector3D direction, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
        {
            ...
            if (Math3D.GetClosestPoints_Line_Cylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
            {
                currentClickPoint = nearestCylinderPoints[0];

                if (Math3D.GetClosestPoints_Line_Cylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, mouseDownClickRay.Origin, mouseDownClickRay.Direction, Math3D.RayCastReturn.ClosestToRay))
                {
                    mouseDownClickPoint = nearestCylinderPoints[0];

                    if (Math3D.GetClosestPoints_Line_Cylinder(out nearestCylinderPoints, out nearestLinePoints, point, direction, radius, mouseDownCenterRay.Origin, mouseDownCenterRay.Direction, Math3D.RayCastReturn.ClosestToRay))
                    {
            ...
            Point3D modelCenter = Math3D.GetClosestPoint_Point_Line(point, direction, currentClickPoint.Value);
        ...
        private static Point3D? CastRay_Sphere(Point3D center, double radius, RayHitTestParameters mouseDownClickRay, RayHitTestParameters mouseDownCenterRay, RayHitTestParameters currentClickRay)
        {
            ...
            Math3D.GetClosestPoints_Line_Sphere(out nearestSpherePoints, out nearestLinePoints, center, radius, currentClickRay.Origin, currentClickRay.Direction, Math3D.RayCastReturn.ClosestToRay);
        ...
        private static Point3D? CastRay_LinesCircles(IEnumerable<RayHitTestParameters> lines, IEnumerable<CircleDefinition> circles, Point3D testPoint)
        {
            ...
            if (lines != null)
            {
                ...
                {
                    Point3D point = Math3D.GetClosestPoint_Point_Line(line.Origin, line.Direction, testPoint);
            ...
            if (circles != null)
            {
                ...
                {
                    Point3D? point = Math3D.GetClosestPoint_Point_Circle(circle.Plane, circle.Center, circle.Radius, testPoint);
                    if (point != null)
                    {
        ...
        private static Point3D? CastRay_LinesCircles(IEnumerable<RayHitTestParameters> lines, IEnumerable<CircleDefinition> circles, RayHitTestParameters ray)
        {
            ...
            if (circles != null)
            {
                ...
                foreach (CircleDefinition circle in circles)
                {
                    ...
                    if (Math3D.GetClosestPoints_Line_Circle(out points1, out points2, circle.Plane, circle.Center, circle.Radius, ray.Origin, ray.Direction, Math3D.RayCastReturn.ClosestToRay))
                    {
        ...
        private static Quaternion GetRotation_Circle(Point3D center, Vector3D normal, Point3D from, Point3D to)
        {
            ...
            Vector3D fromLine = from - Math3D.GetClosestPoint_Point_Line(center, normal, from);
            Vector3D toLine = to - Math3D.GetClosestPoint_Point_Line(center, normal, to);
