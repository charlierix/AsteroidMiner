...
namespace Game.Newt.HelperClasses
{
    public static class Math2D
    {
        ...
        private static class VoronoiUtil
        {
            ...
            private static int[][] GetEdgesByPoint(Point[] points, Edge2D[] edges)
            {
                ...
                for (int cntr = 0; cntr < points.Length; cntr++)
                {
                    retVal[cntr] = GetEdgesByPointSprtPoint(points[cntr], edges, touchingEdges);
                }

                //  Exit Function
                return retVal;
            }
            private static int[] GetEdgesByPointSprtPoint(Point point, Edge2D[] edges, int[][] touchingEdges)
            {
                //  Get the distance to each edge
                int closestIndex = GetEdgesByPointSprtFindFirst(point, edges);
                if (closestIndex < 0)
                {
                ...
                while (true)
                {
                    ...
                    closestIndex = GetEdgesByPointSprtFindNext(point, closestIndex, edges, touchingEdges, retVal, rays.Count > 0);
                    if (closestIndex < 0)
                    {
                ...
                if (rays.Count > 0)
                {
                    return GetEdgesByPointSprtReorder(retVal.ToArray(), rays, touchingEdges);
                }
                else
                {
                    return retVal.ToArray();
            ...
            private static int GetEdgesByPointSprtFindFirst(Point point, Edge2D[] edges)
            {
            ...
            private static int GetEdgesByPointSprtFindNext(Point point, int current, Edge2D[] edges, int[][] touchingEdges, List<int> returnList, bool hasRays)
            {
            ...
            private static int[] GetEdgesByPointSprtReorder(int[] returnList, List<int> rays, int[][] touchingEdges)
            {
                ...
                if (Math.Abs(first - last) > 1)
                {
                    ...
                    return GetEdgesByPointSprtReorderJumbled(returnList, rays[0], rays[1], touchingEdges);
                }
            ...
            private static int[] GetEdgesByPointSprtReorderJumbled(int[] returnList, int ray0, int ray1, int[][] touchingEdges)
            {
                List<int> retVal = new List<int>();
                ...
                for (int cntr = 0; cntr < returnList.Length - 2; cntr++)
                {
                    ...
                    current = GetEdgesByPointSprtReorderJumbledSprtNext(returnList, retVal, touchingEdges[current]);
            ...
            private static int GetEdgesByPointSprtReorderJumbledSprtNext(int[] oldReturn, List<int> newReturn, int[] touching)
            {
            ...
            private static NewCell RebuildCell(int index, RebuildVars vars)
            {
                ...
                var unbound = vars.UnboundControlPoints.Where(o => o.Index == index).FirstOrDefault();
                if (unbound != null)
                {
        ...
        internal static class QuickHull2D
        {
            public static QuickHull2DResult GetConvexHull(Point3D[] points)
            {
                ...
                else
                {
                    ...
                    points2D = GetRotatedPoints(out transformTo2D, out transformTo3D, points);
                }
            ...
            /// </summary>
            private static Point[] GetRotatedPoints(out Transform3D transformTo2D, out Transform3D transformTo3D, Point3D[] points)
            {
                ...
                var transform = GetTransformTo2D(triangle);
            ...
            /// </summary>
            private static Point[] GetRotatedPoints(out Transform3D transformTo2D, out Transform3D transformTo3D, Point3D point1, Point3D point2)
            {
                // Get rotation
        ...
        public static ITriangle GetThreeCoplanarPoints(Point3D[] points)
        {
            Vector3D? line1 = null;
        ...
        public static TriangleIndexed[] GetDelaunayTriangulation(Point[] points, Point3D[] points3D)
        {
            ...
            for (i = 0; i < count; i++)		//NOTE: When a dupe is found, both i and count are decremented
            {
                ...
                for (j = 0; j < i; j++)
                {
                    ...
                    {
                        //i--; count--; dupeCount++;
                        throw new ApplicationException(string.Format("Found duplicate points: {0}, {1} - {2}, {3}"));
                    }
                }
            ...
            pt3[0] = i;
            numTriangles = 1;
            GetDelaunayTriangulationSprtCircum(
            ...
            for (i = 0; i < count; i++)
            {
                ...
                for (j = 0; j < numEd; j++)
                {
                    if ((ed1[j] >= 0) && (ed2[j] >= 0))
                    {
                        ...
                        status = GetDelaunayTriangulationSprtCircum(
        ...
        public static TriangleIndexed[] GetTrianglesFromConvexPoly(Point3D[] polygon)
        {
            ...
            for (int cntr = 1; cntr < polygon.Length - 1; cntr++)
            {
                retVal.Add(new TriangleIndexed(0, cntr, cntr + 1, polygon));
            }
        ...
        private static bool GetDelaunayTriangulationSprtCircum(double x1, double y1, double x2, double y2, double x3, double y3, ref double xc, ref double yc, ref double r)
        {
    ...
    public class Voronoi2DResult
    {
        ...
        public Voronoi2DResult(Point[] controlPoints, Point[] edgePoints, Edge2D[] edges)
            : this(controlPoints, edgePoints, edges, null) { }
    ...
    public class Edge2D
    {
        ...
        /// (this is helpful if you just want to treat the edge like a ray or line)
        /// </summary>
        ...
        public static bool IsTouching(Edge2D edge0, Edge2D edge1)
        {
            ...
            //        (edge1.Index1.Value == edge0.Index0 || edge1.Index1.Value == edge0.Index1.Value))
            //    {
            //        return true;
            //    }
            //}
