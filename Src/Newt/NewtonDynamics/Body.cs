...
namespace Game.Newt.NewtonDynamics
{
	...
	/// </summary>
	public class Body : IDisposable, IComparable<Body>
	{
		#region Events
		...
		public EventHandler<BodyApplyForceAndTorqueArgs> _applyForceAndTorque = null;
		...
		public Body(CollisionHull hull, Matrix3D offsetMatrix, double mass, Visual3D[] visuals)
		{
			_world = hull.World;
			...
			{
				//	For some reason, this is coming back with a new handle for compound collision hulls
				ObjectStorage.Instance.AddCollisionHull(hullHandlePost, hull);
			...
			this.Token = ObjectStorage.Instance.GetNextToken();
