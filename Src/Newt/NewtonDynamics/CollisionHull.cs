...
namespace Game.Newt.NewtonDynamics
{
	...
	public class CollisionHull : IDisposable
	{
		...
		protected CollisionHull(IntPtr handle, WorldBase world, float[] offsetMatrix, CollisionShapeType collisionShape)
		{
			...
			//if (_collisionShape != CollisionShapeType.Null)
			//{
		...
		private void Dispose(bool disposing)
		{
			...
			//	This makes it very dangerous to dispose hulls when there are still bodies around
		...
		public static CollisionHull CreateConvexHull(WorldBase world, int shapeID, IEnumerable<Point3D> verticies, double tolerance, Matrix3D? offsetMatrix)
		{
		...
		public void Release()
		{
			//TODO:  Should this be dispose instead?
		...
		public bool GetNearestPoint_PointToHull(out Point3D? contactPoint, Vector3D? normal, Vector3D point, int threadIndex)
		{
			...
			int retVal = Newton.NewtonCollisionPointDistance(_world.Handle, new NewtonVector3(point).Vector, _handle, _offsetMatrix, contactNewt.Vector, normalNewt.Vector, threadIndex);
		...
		public bool GetNearestPoint_HullToHull(out Point3D? contactPointThis, out Point3D? contactPointOther, out Vector3D? normal, CollisionHull otherHull, int threadIndex)
		{
			...
			int retVal = Newton.NewtonCollisionClosestPoint(_world.Handle, _handle, _offsetMatrix, otherHull.Handle, otherHull._offsetMatrix, contactANewt.Vector, contactBNewt.Vector, normalNewt.Vector, threadIndex);
		...
		public List<IntersectionPoint> GetIntersectingPoints_HullToHull(int maxReturnCount, CollisionHull otherHull, int threadIndex)
		{
			...
			//	Call Newton
			int pointCount = Newton.NewtonCollisionCollide(_world.Handle, maxReturnCount, _handle, _offsetMatrix, otherHull.Handle, otherHull._offsetMatrix, contacts, normals, penetrationDistances, threadIndex);
		...
		public List<IntersectionPoint> GetIntersectingPoints_HullToHull(int maxReturnCount, double timestep, Vector3D velocity, Vector3D angularVelocity, CollisionHull otherHull, Vector3D otherVelocity, Vector3D otherAngularVelocity, int threadIndex)
		{
			//	Prep some arrays to hold the return values
			float[] contacts = new float[3 * maxReturnCount];
			float[] normals = new float[3 * maxReturnCount];
			...
			int pointCount = Newton.NewtonCollisionCollideContinue(_world.Handle, maxReturnCount, Convert.ToSingle(timestep),
																												_handle, _offsetMatrix, new NewtonVector3(velocity).Vector, new NewtonVector3(angularVelocity).Vector,
																												otherHull.Handle, otherHull._offsetMatrix, new NewtonVector3(otherVelocity).Vector, new NewtonVector3(otherAngularVelocity).Vector,
		...
		public bool RayCast(out double percentAlongLine, out Point3D? contactPoint, out Vector3D? contactNormal, out int faceID, Point3D startPoint, Point3D endPoint)
		{
			...
			{
				contactPoint = ((endPoint - startPoint) * percentAlongLine).ToPoint();
		...
		public void CalculateAABB(out Point3D minPoint, out Point3D maxPoint)
		{
			NewtonVector3 minNewt = new NewtonVector3();
			NewtonVector3 maxNewt = new NewtonVector3();

			Newton.NewtonCollisionCalculateAABB(_handle, _offsetMatrix, minNewt.Vector, maxNewt.Vector);
